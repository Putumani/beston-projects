...
function Projects({ navigateTo }) {
  ...
  return (
    <>
      <section className="w-full py-24 select-none relative z-10" style={{ backgroundColor: brandDark }}>
        <div className="w-full mx-auto px-6 md:px-10 lg:px-[50px]" style={{ maxWidth: '1440px' }}>
          ...
          <div className="text-center max-w-3xl mx-auto mb-20">
            <div className="flex items-center justify-center gap-2 mb-3">
              ...
              <p className="text-xs font-bold uppercase tracking-[0.2em] text-gray-300">Recent Projects</p>
            </div>
          ...
          <div className="space-y-12 max-w-5xl mx-auto mb-16">
            {projectsData.map((project, idx) => {
              ...
              return (
                <div key={idx} className={`w-full bg-[#0c2556] rounded-[32px] overflow-hidden p-6 md:p-8 flex flex-col lg:flex-row items-center gap-8 lg:gap-12 border border-slate-800 transition-transform duration-300 hover:-translate-y-1 ${isEven ? 'lg:flex-row' : 'lg:flex-row-reverse'}`}>
                  <div className="w-full lg:w-1/2 relative h-[260px] sm:h-[320px] rounded-2xl overflow-hidden bg-slate-900 group">
                    <div className={`absolute inset-0 w-full h-full transition-transform duration-700 group-hover:scale-105 overflow-hidden ${isEven ? 'rounded-tl-[45px]' : 'rounded-tr-[45px]'}`}>
                  ...
                  <div className="w-full lg:w-1/2 flex flex-col justify-center">
                    ...
                    <p className="text-slate-300 text-sm sm:text-[15px] font-medium leading-relaxed mb-6 opacity-95">{project.description}</p>
                    ...
                    <div>
                      <button 
                        ...
                        className="inline-flex items-center gap-2 text-xs sm:text-sm font-bold tracking-wide transition-colors group bg-transparent border-none cursor-pointer" 
          ...
          <div className="flex justify-center">
            <button 
              onClick={() => navigateTo('projects')}
              className="text-xs sm:text-sm font-bold text-white px-8 py-3.5 rounded-full shadow-md transition-transform duration-200 hover:scale-[1.02] border-none cursor-pointer"
      ...
      <div className="w-full h-[400px] sm:h-[500px] bg-fixed bg-center bg-no-repeat bg-contain" style={{ backgroundImage: `url(${awardImg})`, backgroundColor: '#ffffff' }} />
    </>
  )
}
